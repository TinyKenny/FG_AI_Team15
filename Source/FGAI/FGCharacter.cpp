...
#include "FGAI/Components/FGHealthComponent.h"
#include "FGAI/Components/FGTakeDamageComponent.h"
...
AFGCharacter::AFGCharacter()
{
	...
	HealthComponent = CreateDefaultSubobject<UFGHealthComponent>(TEXT("HealhtComponent"));
	HealthComponent->OnReachZeroHealth.AddDynamic(this, &AFGCharacter::HandleOnReachedZeroHealth);

	TakeDamageComponent = CreateDefaultSubobject<UFGTakeDamageComponent>(TEXT("TakeDamageComponent"));
}
...
void AFGCharacter::HandleOnReachedZeroHealth()
{
	if (!GameOverLevel.IsNull())
	{
		UGameplayStatics::OpenLevelBySoftObjectPtr(this, GameOverLevel);
	}
	else
	{
		UE_LOG(LogTemp, Log, TEXT("Game over level is null"))
	}
}
