...
void UFGVisionSensingComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (int32 Index = SensedTargets.Num() - 1; Index >= 0; --Index)
	{
		...
		if (!IsPointVisible(Target->GetTargetOrigin(), Origin, Direction, SensingSettings->DistanceMinimum, Target->GetOwner()))
		{
	...
	for (UFGVisionSensingTargetComponent* Target : ListOfTargets)
	{
		if (!SensedTargets.Contains(Target) &&
			IsPointVisible(Target->GetTargetOrigin(), Origin, Direction, SensingSettings->DistanceMinimum, Target->GetOwner()))
		{
...
bool UFGVisionSensingComponent::IsPointVisible(const FVector& PointToTest, const FVector& Origin, const FVector& Direction, float DistanceMinimum, const AActor* PointOwner) const
{
	if (SensingSettings == nullptr)
		return false;
	...
	{
		FHitResult TraceHit;
		UKismetSystemLibrary::LineTraceSingle(this, Origin, PointToTest, TraceTypeQuery1, false, TArray<AActor*>(), EDrawDebugTrace::None, TraceHit, true);
		if(TraceHit.GetActor() == nullptr || TraceHit.GetActor() == PointOwner)
		{
			return true;
		}
	}
